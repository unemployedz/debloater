function findValues(value, keys, out = []) {
  if (!value || typeof value !== 'object' || out.length >= 35) return out;
  if (Array.isArray(value)) { for (const item of value) findValues(item, keys, out); return out; }
  for (const [k,v] of Object.entries(value)) {
    if (keys.includes(k)) {
      if (typeof v === 'string' && /^https?:\/\//.test(v)) out.push(v.replace(/\\u002F/g,'/').replace(/\\u0026/g,'&'));
